@php
  $transactions = $entry->transactions->where('is_completed', 1)->sortByDesc('created_at');
@endphp
  ...
  @if(count($transactions))
    ...
    <table>
      ...
      <tr>
        ...
        <td><a href="{{ url('admin/order/' . $transaction->order_id . '/show') }}">{{ $transaction->order? $transaction->order->code : '' }}</a></td>
