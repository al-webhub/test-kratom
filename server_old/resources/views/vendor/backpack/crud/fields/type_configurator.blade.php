...
?>
<!-- enum -->
<div @include('crud::inc.field_wrapper_attributes') id="conf-all-wrapper" data-current-type="">
                ...
                @foreach ($entity_model::getPossibleEnumValues($field['name']) as $possible_value)
                    <option value="{{ $possible_value }}"
                        @if (( old($field['name']) &&  old($field['name']) == $possible_value) || (isset($field['value']) && $field['value']==$possible_value))
                            selected
                        @endif
                    >{{ __('general.fieldType.'.$possible_value) }}</option>
                @endforeach
            @endif
    </select>
	   ...
	   $(document).ready(function(){
          ...
          'color' : 'color',
		   ...
		   function openFieldOptions(fieldType){
			   ...
			   $('.conf-field-wrapper').find('input, select, textarea').attr('disabled', 'disabled').end().filter('[data-field-type = "'+fieldType+'"]').find('input, select, textarea').removeAttr('disabled');
		   }
