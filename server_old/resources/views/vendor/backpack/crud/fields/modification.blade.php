@php
  $base = isset($entry) && $entry->baseModification->count() ? $entry->baseModification : null;
...
<div class="form-group col-sm-12">
  ...
  <div class="form-group col-sm-12" v-for="(item, index) in baseItem.extras.complectations">
    ...
    <div class="array-controls btn-group m-t-10">
        <button class="btn btn-sm btn-light" type="button" @click="addComplectation()"><i class="fa fa-plus"></i> Добавить комплектацию</button>
    </div>
  </div>
</div>
@endif
...
<div class="@if(config('aimix.shop.enable_modifications')) default-modifications @endif">
  @if(config('aimix.shop.enable_modifications'))
  <h4>Значения по умолчанию</h4>
  @endif
{{--  <input type="hidden" name="{{ $field['name'] }}[0][language_abbr]" value="{{ $crud->request->input('locale') }}"> --}}
...
<div class="form-group col-sm-12">
  ...
  <div class="row">
    ...
    <div class="col-8 col-sm-6 col-xl-4 mb-3">
      <button class="btn btn-block btn-primary" type="button" @click="addItem()">Добавить модификацию</button>
    </div>
  </div>
  ...
  <div class="card" v-for="(item, index) in items" >
    <div class="card-header d-flex">
		<input type="hidden" :name="'{{ $field['name'] }}[' + (index + 1) + '][language_abbr]'" value="{{ $crud->request->input('locale') }}">
        ...
        <div class="col-4 col-sm-2 col-xl-1 mb-3">
          <a href="#" class="card-header-action btn-minimize btn btn-light h-100 w-100 d-flex align-items-center justify-content-center" @click.prevent="toggleTab(index)"><i class="fa fa-angle-down"></i></a>
        </div>
    </div>
    ...
    <!-- MODIFICATION END -->
  <template v-else>
    <div class="col-12 text-center">Модификации отсутствуют</div>
  </template>
...
var baseItem = @json(isset($entry) && $entry->baseModification->count()? $entry->baseModification : $base_default);
var items = @json(isset($entry) ? $entry->notBaseModifications->get() : []);
